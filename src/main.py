...
class FocusFlowApp:
    def __init__(self, root):
        ...
        # UI Elements
        tk.Label(
            root, text="O que vamos estudar agora?", font=("Arial", 12)
        ).pack(pady=10)
        ...
        self.btn_start = tk.Button(
            root, text="Iniciar Foco", command=self.start_timer, bg="green", fg="white"
            )
        self.btn_start.pack()
    ...
    def update_timer(self):
        if self.timer_running and self.seconds_left > 0:
            ...
            self.timer_running = False
            messagebox.showinfo(
                "Parabéns!", "Ciclo de estudos finalizado! Hora de uma pausa.")
